...
def run_n_vacc(distance):
    ...
    vac = "Unequal" #only 
    initial_states = "Unequal"  
    specification = f"distance_{distance}"
    path = (
...
with mp.Pool() as p:
    results = list(
        tqdm.tqdm(
            p.imap_unordered(
                run_n_vacc, np.array(list(np.linspace(0.015, 0.167, 48)) + list(np.linspace(0.2, 1, 20)))
            ),
            total=11,
        )
    )
...
path = (
        f"/home/manuel/Documents/VaccinationDistribution/code/objects/{specification}_distance_3.pkl"
        )

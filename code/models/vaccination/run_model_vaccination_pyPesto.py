...
from models.vaccination.parameter import parameters_vaccine_two_toy
...
def criterion(
    ...
    set_fixed_parameter = {**set_fixed_parameter, **parameters_vaccine_two_toy()}
...
def sensitivity_parallel(distance):
    ...
    d_dict = {
        "distance_countryA_countryB": distance,
        "distance_countryB_countryA": distance,
    }
    ...
    if successes != 0:
        problem = pypesto.Problem(
            objective=objective, lb=lb, ub=ub, x_guesses=start_matrix
        )
    ...
    objective_unrestricted_sensitivity = pypesto.Objective(
        fun=func_to_optimize_unrestricted_sensitivity
    )
    problem_unrestricted = pypesto.Problem(
        objective=objective_unrestricted_sensitivity, lb=lb, ub=ub
    )
...
equi_grid_small = np.linspace(0.0000001, 0.001, 40) ** -1 - 1
equi_grid_medium = np.linspace(0.00101, 0.05, 20) ** -1 - 1
equi_grid_large = np.linspace(0.0501, 1, 20) ** -1 - 1

grid = np.concatenate((equi_grid_small, equi_grid_medium, equi_grid_large))
...
with open(
    "/home/manuel/Documents/VaccinationDistribution/code/objects/output_splines_sensitivity_distance_new.pkl",
    "wb",
) as output:
    out = df

...
first_time = False
# --------------------------Create Model--------------------------------------
model_name = "vaccination_partly"
path_sbml = "stored_models/vaccination_partly/" + model_name
...
xx_names_str = []
for index in range(number_yy):
    xx_names_str.append(f"xx{index}")

yy_current_splines = np.repeat(0.0, len(yy_names_str))
yy_current = dict(zip(yy_names_str, yy_current_splines))

xx_full = splines["spline_countryA_vac1"]["xx"]


def get_periods(xx_full, number_periods, max_grid=4000):

    grid = [float(xx_full[0])]

    if number_periods > 2:
        for index in range(number_periods - 2):
            grid.append(float(xx_full[index + 1]))

    grid.append(float(xx_full[-1]))

    length_xx = len(xx_full)

    for index in range(number_periods, length_xx):
        grid.append(max_grid + index)
...
xx_periods = get_periods(xx_full, 2)
xx_dict = dict(zip(xx_names_str, xx_periods))

set_parameter_current = {**set_fixed_parameter, **yy_current, **xx_dict}
...
def criterion(
    ...
    fixed_params = {**fixed_parameter(), **parameters_vaccine_two()}
    set_parameter = {**set_fixed_parameter, **set_proportions, **fixed_params}
...
optimal_dict = {}
multistarts_dict = {}
for periods_splines in range(2, number_yy + 1):

    xx_periods = get_periods(xx_full, periods_splines)
    xx_dict = dict(zip(xx_names_str, xx_periods))
    yy_names_str_theta = (
        yy_names_str[0 : (periods_splines - 1)]
        + [yy_names_str[len(xx_periods) - 1]]
        + yy_names_str[len(xx_periods) : (len(xx_periods) + periods_splines - 1)]
        + [yy_names_str[-1]]
    )
    ...
    to_optimize = partial(
        ...
        model=model,
        solver=solver,
        set_fixed_parameter={**set_fixed_parameter, **xx_dict},
        set_start_parameter=start_parameter_two(),
        yy_names=yy_names_str_theta,
    ...
    number_parameters = periods_splines * number_vaccines

    cols = (
        ["start_" + yy_parameter for yy_parameter in yy_names_str_theta]
        + ["theta_" + yy_parameter for yy_parameter in yy_names_str_theta]
        + ["value"]
    )
    ...
    for index_multi in range(n_multi):
        print(f"index: {index_multi}; spline parameter: {periods_splines}")
        if periods_splines == 2:
            start = np.random.uniform(lower_bound, upper_bound, number_parameters)
        else:
            prev_solution = list(
                optimal_dict[f"splines_{periods_splines-1}"]["solution_x"]
            )
            random_one = np.random.uniform(lower_bound, upper_bound, 1)[0]
            random_two = np.random.uniform(lower_bound, upper_bound, 1)[0]
            prev_solution.insert(periods_splines, random_one)
            prev_solution.append(random_two)
            start = prev_solution

        start_params = pd.DataFrame(
            data=start,
            columns=["value"],
            index=yy_names_str_theta,
        )
    ...
    multistarts_dict[f"splines_{periods_splines}"] = df_multistart
    optimal_dict[f"splines_{periods_splines}"] = res_optimal
...
yy_optimal = dict(zip(yy_names_str_theta, transformed_theta_optimal))
...
# -----------------------Plot Optimum--------------------------------------------
plot_path = "/home/manuel/Documents/VaccinationDistribution/paper/images/iterative"

...
from models.vaccination.create_model_vaccination import (
    get_all_individuals_to_be_vaccinated,
)
...
vaccine_supply_df = model_object["observables"][
    [
...
def get_nu_parameter(
    ...
    for index_vaccination in vaccination_states_removed:
        for index_areas in areas:
            ...
            formula = f"vaccine_supply_df.loc[vaccine_supply_df[str('time')] == tau, str('observable_quantity_{index_areas}_{index_vaccination}')] / ({to_be_vaccinated})"
            nu_parameter[id_nu] = formula
    return nu_parameter


nu_parameter = get_nu_parameter(
    species,
    vaccination_states_removed,
    areas,
    vaccinated_compartments,
...
def create_formulas_eval(
    dictionary, species, par_dict, name_dict="par_dict", name_df="Y"
):
    ...
    for index_species in species:
        for index_reactions in dictionary.keys():
            ...
            if index_species in formula:
                new = f"{name_df}[str('{index_species}')][period]"
                dictionary[index_reactions]["formula"] = formula.replace(
                    index_species, new
                )
...
formulas = create_formulas_eval(
    reactions, species, par_dict, name_dict="par_dict", name_df="Y"
)
...
with mp.Pool() as p:
    results = list(
        tqdm.tqdm(
            p.imap_unordered(run_tau_leaping_parallel, range(500, 1000)), total=500
        )
    )

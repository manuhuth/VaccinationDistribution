...
%\input{tikz/splines}
Figure \ref{fig:examples} shows an exemplary spline (second plot) and how it is shrunk into the unit interval via the logistic function (third plot). %Within the sensitivity analysis we use the hyperbolic tangent function $\tanh(x)$ to examine if the functional choice of the transformation changes our results.
